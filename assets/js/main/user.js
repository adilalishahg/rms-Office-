...
function delete_employee(id,route='true') {
	$.ajax({
		type: "POST",
...
function preventFormProfile(e) {
	...
	$.ajax({
		...
		success: function (response) {
			...
			if (res.status == "img_error") {
				swal(res.errors.replace(/<\/?p>/g, '') + "!", res.errors.replace(/<\/?p>/g, ''), "error");
				showError(res.errors);
				// $(".error-message").hide();
			}
		},
...
function edit_employee(id,type='') {
	$.ajax({
		type: "POST",
		url: "employees_ajax", // Replace with your server endpoint
		data: {
...
function edit_employee_type(id,type='') {
	$.ajax({
		type: "POST",
		url: "worker_type_ajax", // Replace with your server endpoint
		data: {
			id: id,
		},
		success: function (response) {
...
function user_edit_ajax(data,formatType='') {
	if(formatType==''){
